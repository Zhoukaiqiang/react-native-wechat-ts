...
public class WechatTsModule extends ReactContextBaseJavaModule {
  ...
  /**
   * fix Native module WeChatTsModule tried to override WeChatTsModule for module name RCTWeChat.
   * If this was your intention, return true from WeChatTsModule#canOverrideExistingModule() bug
   *
   * @return
   */
  ...
  private static final ArrayList<WechatTsModule> modules = new ArrayList<>();
  ...
  public static void handleIntent(Intent intent) {
    for (WechatTsModule mod : modules) {
    ...
    if (uri != null) {
      this._getImage(uri, new ResizeOptions(100, 100), new ImageCallback() {
        ...
        public void invoke(@Nullable Bitmap bitmap) {
          WechatTsModule.this._share(scene, data, bitmap, callback);
        }
      });
    } else {
      ...
      __jsonToImageUrlMedia(data, new MediaObjectCallback() {
        ...
        public void invoke(@Nullable WXMediaMessage.IMediaObject mediaObject) {
          if (mediaObject == null) {
            ...
            WechatTsModule.this._share(scene, data, thumbImage, mediaObject, callback);
          }
        }
      });
      return;
